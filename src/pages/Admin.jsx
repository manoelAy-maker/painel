...
export default function Admin() {
  ...
  const salvarFuncionario = async () => {
    if (!funcionario.nome.trim() || !funcionario.filial || !funcionario.cargo) {
      alert('Preencha nome, cargo e filial.')
      return
    }

    if (editandoUsuario) {
      const ok = await editarUsuario(editandoUsuario, funcionario)
      if (ok) limparFuncionario()
      return
    }

    if (!funcionario.usuario.trim() || !funcionario.senha.trim()) {
      alert('Preencha login e senha para criar o colaborador.')
      return
    }

    const ok = await criarUsuario({
      ...funcionario,
  ...
  return (
    <section className="aba active users-pro-page">
      <div className="users-pro-hero">
        <div>
          <span className="users-pro-eyebrow">Acessos do sistema</span>
          <h2>Usuários e cargos</h2>
          <p>Gerencie colaboradores, permissões e separação por filial sem misturar operações.</p>
        </div>
        ...
        <div className="users-pro-card users-pro-form-card">
          <div className="users-pro-card-head">
            <div>
              ...
              <p>{editandoUsuario ? `Editando acesso de ${editandoUsuario}` : 'Cadastre acesso, cargo e filial de atuação.'}</p>
            </div>
          ...
          <div className="users-pro-form">
            <label>
              <span>Nome</span>
              <input value={funcionario.nome} onChange={e => setFuncionario(p => ({ ...p, nome: e.target.value }))} placeholder="Nome completo" />
            </label>
            <label>
              <span>Login</span>
              <input disabled={!!editandoUsuario} value={funcionario.usuario} onChange={e => setFuncionario(p => ({ ...p, usuario: normalizarLogin(e.target.value) }))} placeholder="usuario" />
            </label>
            <label>
              <span>{editandoUsuario ? 'Nova senha opcional' : 'Senha'}</span>
              <input type="password" value={funcionario.senha} onChange={e => setFuncionario(p => ({ ...p, senha: e.target.value }))} placeholder={editandoUsuario ? 'Deixe vazio para manter' : 'Senha inicial'} />
            </label>
            <label>
              <span>Cargo</span>
              <select value={funcionario.cargo} onChange={e => setFuncionario(p => ({ ...p, cargo: e.target.value }))}>
          ...
          <div className="users-pro-filial-box">
            <div className="users-pro-label-row">
              <span>Filial / etiqueta de acesso</span>
              {funcionario.filial === FILIAL_OLEO && <b>Operação do Óleo</b>}
            </div>
            <select value={funcionario.filial} onChange={e => selecionarFilial(e.target.value)}>
              <option value="">Selecione...</option>
              {filiaisBase.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}
            </select>
            <div className="users-pro-tags">
              {filiaisBase.map(f => (
                <button key={f.id} className={funcionario.filial === f.id ? 'active' : ''} onClick={() => selecionarFilial(f.id)}>
                  {f.id === FILIAL_OLEO ? 'Operação do Óleo' : f.nome}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="users-pro-actions">
            <button className="users-pro-primary" onClick={salvarFuncionario}>{editandoUsuario ? 'Salvar alterações' : 'Criar colaborador'}</button>
            <button className="users-pro-ghost" onClick={garantirFilialOleo}>Aplicar óleo</button>
          </div>
        </div>
        ...
        <div className="users-pro-card users-pro-rules-card">
          <div className="users-pro-card-head">
            <div>
              ...
              <p>Separação de dados por filial.</p>
            </div>
          </div>
          <div className="users-pro-rule-list">
            <div><strong>Usuário Jataí</strong><span>Visualiza registros da filial configurada.</span></div>
            <div><strong>Usuário do Óleo</strong><span>Acessa somente registros com filial <code>oleo</code>.</span></div>
            <div><strong>Admin geral</strong><span>Consegue administrar colaboradores e filiais.</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="users-pro-card">
        <div className="users-pro-card-head users-pro-list-head">
          <div>
            <h3>Colaboradores</h3>
            <p>{funcionariosFiltrados.length} registro(s) encontrado(s)</p>
          </div>
          ...
          <table className="users-pro-table">
            ...
            <tbody>
              {funcionariosFiltrados.map(u => (
                <tr key={u.usuario}>
                  <td>
                    <div className="users-pro-person">
                      <span>{u.avatar || iniciais(u.nome)}</span>
                  ...
                  <td>
                    <div className="users-pro-row-actions">
                      ...
                      {u.usuario !== ADMIN_USERNAME && <button className="danger" onClick={() => confirm(`Excluir ${u.usuario}?`) && excluirUsuario(u.usuario)}>Excluir</button>}
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <div className="users-pro-card">
        <div className="users-pro-card-head users-pro-list-head">
          <div>
            <h3>Filiais</h3>
            <p>{filiaisBase.length} filial(is) cadastrada(s)</p>
          </div>
        </div>
        <div className="users-pro-branch-grid">
          {filiaisBase.map(f => (
            <div key={f.id} className="users-pro-branch">
              <div><strong>{f.nome}</strong><span>{f.cidade}{f.estado ? ` — ${f.estado}` : ''}</span><code>{f.id}</code></div>
              {f.id !== 'jatai-go' && f.id !== 'mineiros-go' && f.id !== FILIAL_OLEO && <button className="danger" onClick={() => confirm(`Excluir filial ${f.nome}?`) && excluirFilial(f.id)}>Excluir</button>}
            </div>
          ))}
        </div>
        <div className="users-pro-form branch-form">
          <label><span>ID da filial</span><input value={novaFilial.id} onChange={e => setNovaFilial(p => ({ ...p, id: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') }))} placeholder="ex: jatai-go" /></label>
          <label><span>Nome</span><input value={novaFilial.nome} onChange={e => setNovaFilial(p => ({ ...p, nome: e.target.value }))} placeholder="Via Log Jataí" /></label>
          <label><span>Cidade</span><input value={novaFilial.cidade} onChange={e => setNovaFilial(p => ({ ...p, cidade: e.target.value }))} placeholder="Jataí" /></label>
          <label><span>Estado</span><input value={novaFilial.estado} onChange={e => setNovaFilial(p => ({ ...p, estado: e.target.value.toUpperCase() }))} placeholder="GO" maxLength={2} /></label>
        </div>
