...
export default function Admin() {
  ...
  return (
    <section className="aba active users-pro-page">
      <div className="users-pro-hero"><div><span className="users-pro-eyebrow">Acessos do sistema</span><h2>Usuários e cargos</h2><p>Gerencie colaboradores, permissões, localização de acesso e separação por filial sem misturar operações.</p></div><button className="users-pro-ghost" onClick={limparFuncionario}>Novo colaborador</button></div>
      ...
      <div className="users-pro-grid">
        <div className="users-pro-card users-pro-form-card">
          <div className="users-pro-card-head"><div><h3>{editandoUsuario ? 'Editar colaborador' : 'Novo colaborador'}</h3><p>{editandoUsuario ? `Editando acesso de ${editandoUsuario}` : 'Cadastre acesso, cargo e filial de atuação.'}</p></div>{editandoUsuario && <button className="users-pro-ghost small" onClick={limparFuncionario}>Cancelar</button>}</div>
          ...
          <div className="users-pro-filial-box"><div className="users-pro-label-row"><span>Filial / etiqueta de acesso</span>{funcionario.filial === FILIAL_OLEO && <b>Operação do Óleo</b>}</div><select value={funcionario.filial} onChange={e => selecionarFilial(e.target.value)}><option value="">Selecione...</option>{filiaisBase.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}</select><div className="users-pro-tags">{filiaisBase.map(f => <button key={f.id} className={funcionario.filial === f.id ? 'active' : ''} onClick={() => selecionarFilial(f.id)}>{f.id === FILIAL_OLEO ? 'Operação do Óleo' : f.nome}</button>)}</div></div>
          <div className="users-pro-actions"><button className="users-pro-primary" onClick={salvarFuncionario}>{editandoUsuario ? 'Salvar alterações' : 'Criar colaborador'}</button><button className="users-pro-ghost" onClick={garantirFilialOleo}>Aplicar óleo</button></div>
        </div>
        <div className="users-pro-card users-pro-rules-card"><div className="users-pro-card-head"><div><h3>Regra de acesso</h3><p>Separação de dados por filial.</p></div></div><div className="users-pro-rule-list"><div><strong>Usuário Jataí</strong><span>Visualiza registros da filial configurada.</span></div><div><strong>Usuário do Óleo</strong><span>Acessa somente registros com filial <code>oleo</code>.</span></div><div><strong>Admin geral</strong><span>Consegue administrar colaboradores, filiais e localizações.</span></div></div></div>
      </div>
      ...
      <div className="users-pro-card"><div className="users-pro-card-head users-pro-list-head"><div><h3>Filiais</h3><p>{filiaisBase.length} filial(is) cadastrada(s)</p></div></div><div className="users-pro-branch-grid">{filiaisBase.map(f => <div key={f.id} className="users-pro-branch"><div><strong>{f.nome}</strong><span>{f.cidade}{f.estado ? ` — ${f.estado}` : ''}</span><code>{f.id}</code></div>{f.id !== 'jatai-go' && f.id !== 'mineiros-go' && f.id !== FILIAL_OLEO && <button className="danger" onClick={() => confirm(`Excluir filial ${f.nome}?`) && excluirFilial(f.id)}>Excluir</button>}</div>)}</div><div className="users-pro-form branch-form"><label><span>ID da filial</span><input value={novaFilial.id} onChange={e => setNovaFilial(p => ({ ...p, id: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') }))} placeholder="ex: jatai-go" /></label><label><span>Nome</span><input value={novaFilial.nome} onChange={e => setNovaFilial(p => ({ ...p, nome: e.target.value }))} placeholder="Via Log Jataí" /></label><label><span>Cidade</span><input value={novaFilial.cidade} onChange={e => setNovaFilial(p => ({ ...p, cidade: e.target.value }))} placeholder="Jataí" /></label><label><span>Estado</span><input value={novaFilial.estado} onChange={e => setNovaFilial(p => ({ ...p, estado: e.target.value.toUpperCase() }))} placeholder="GO" maxLength={2} /></label></div><button className="users-pro-primary slim" onClick={adicionarFilial}>Adicionar filial</button></div>
    </section>
  )
}
