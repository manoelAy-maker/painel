...
function ModalMotorista({ aberto, fechar, salvarMotorista, editando }) {
  ...
  const confirmar = () => {
    if (!form.nome.trim() || !form.numero.trim()) {
      alert('Informe nome e telefone do motorista.')
      return
    }
  ...
  return (
    <div className="cap2-modal-backdrop" onClick={fechar}>
      <div className="cap2-modal" onClick={(e) => e.stopPropagation()}>
        <div className="cap2-modal-head">
          <div>
            <span>{editando ? 'Editar captação' : 'Nova captação'}</span>
            <h2>{editando ? 'Atualizar motorista' : 'Cadastrar motorista'}</h2>
          </div>
        ...
        <div className="cap2-choice-block">
          <label>Operação</label>
          <div className="cap2-choice-row">{OPERACOES.map((op) => <button key={op} onClick={() => set('operacao', op)} className={form.operacao === op ? 'active' : ''}>{op}</button>)}</div>
        </div>

        <div className="cap2-choice-block">
          <label>Status</label>
          <div className="cap2-status-row">{Object.entries(STATUS).map(([key, s]) => <button key={key} onClick={() => set('status', key)} className={form.status === key ? 'active' : ''} style={form.status === key ? { borderColor: s.cor, color: s.cor } : {}}>{s.icon} {s.label}</button>)}</div>
        </div>

        {form.status === 'nao_carregou' && (
          <div className="cap2-no-load">
            <div className="cap2-field"><label>Motivo</label><select value={form.motivoNaoCarregou} onChange={(e) => set('motivoNaoCarregou', e.target.value)}><option value="">Selecione...</option>{MOTIVOS_NAO_CARREGOU.map((m) => <option key={m}>{m}</option>)}</select></div>
            <div className="cap2-choice-block"><label>Impacto</label><div className="cap2-impact-row">{Object.entries(IMPACTO).map(([key, info]) => <button key={key} onClick={() => set('impactoPontuacao', key)} className={form.impactoPontuacao === key ? 'active' : ''} style={form.impactoPontuacao === key ? { borderColor: info.cor, color: info.cor } : {}}><strong>{info.icon} {info.label}</strong><small>{info.curto}</small></button>)}</div></div>
            <div className="cap2-field"><label>Justificativa</label><textarea value={form.justificativaNaoCarregou} onChange={(e) => set('justificativaNaoCarregou', e.target.value)} placeholder="Explique o motivo rapidamente." /></div>
          </div>
        )}
...
export default function Captacao() {
  ...
  const lista = useMemo(() => base
    ...
    .filter((m) => !busca || [m.nome, m.numero, m.operacao, STATUS[m.status]?.label, m.obs, m.motivoNaoCarregou].join(' ').toLowerCase().includes(busca.toLowerCase()))
  ...
  const avancar = async (m) => {
    const proximo = STATUS[m.status]?.next
    if (!proximo) return
  ...
  return (
    <div className="cap2-shell">
      <section className="cap2-hero">
        <div>
          <span className="cap2-eyebrow">Central de captação</span>
          <h1>Captação de veículos</h1>
          <p>Cadastre motorista em poucos segundos, avance o status e acompanhe o carregamento sem bagunça.</p>
        </div>
        <button onClick={() => { setEditando(null); setModal(true) }} className="cap2-primary">+ Novo lead</button>
      </section>

      <section className="cap2-board">
        <div className="cap2-owner">
          <div className="cap2-avatar">{nomeCaptador[0]?.toUpperCase() || '?'}</div>
          <div><small>{isAdmin ? 'Admin' : 'Captador'}</small><strong>{nomeCaptador}</strong></div>
          <span>{isAdmin ? 'Geral' : nomeFilial(filialAtual)}</span>
        </div>
        <div className="cap2-metrics">
        ...
        <div className="cap2-conversion"><div><span>Taxa de carregamento</span><strong>{conversao}%</strong></div><i><b style={{ width: `${conversao}%` }} /></i></div>
      </section>

      <section className="cap2-toolbar">
        <div className="cap2-search"><span>🔎</span><input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar motorista, telefone ou observação..." /></div>
        <div className="cap2-filters">
      ...
      <section className="cap2-list-card">
        <div className="cap2-list-head"><strong>Leads da captação</strong><span>{lista.length} registro(s)</span></div>
        <div className="cap2-list">
          {lista.length === 0 && <div className="cap2-empty"><strong>Nenhum lead ainda</strong><span>Clique em “Novo lead” para começar a captação.</span></div>}
          {lista.map((m) => {
            const s = STATUS[m.status] || STATUS.contatado
            const isNaoCarregou = m.status === 'nao_carregou'
            return (
              <div key={m.id} className={`cap2-row ${isNaoCarregou ? 'danger' : ''}`}>
                <div className="cap2-status-dot" style={{ color: s.cor, background: `${s.cor}18` }}>{s.icon}</div>
                <div className="cap2-row-main">
                  <div className="cap2-row-title"><strong>{m.nome}</strong><span>{m.operacao}</span>{Number(m.quantidadeCargas || 1) > 1 && <em>{m.quantidadeCargas} cargas</em>}</div>
                  <div className="cap2-row-sub">{m.numero || 'Sem telefone'} · {m.data || '-'} {isAdmin ? `· ${m.nomeCaptador || m.captador}` : ''}</div>
                  {m.obs && <p>{m.obs}</p>}
                  {isNaoCarregou && <div className="cap2-reason"><strong>{m.motivoNaoCarregou || 'Não carregou'}</strong><span>{m.justificativaNaoCarregou || 'Sem justificativa.'}</span><small>{IMPACTO[impactoKey(m.impactoPontuacao)]?.label || 'Motivo externo'}</small></div>}
                </div>
                <div className="cap2-actions">
                  {!['carregou', 'nao_carregou'].includes(m.status) && <button onClick={() => avancar(m)} className="cap2-next">Avançar</button>}
                  {!['carregou', 'nao_carregou'].includes(m.status) && <button onClick={() => marcarNaoCarregou(m)} className="cap2-lost">Não carregou</button>}
                  {m.numero && <a href={abrirWhats(m)} target="_blank" rel="noopener noreferrer">Whats</a>}
                  <button onClick={() => { setEditando(m); setModal(true) }}>Editar</button>
                  <button onClick={() => confirm('Excluir lead?') && excluir(m.id)}>Excluir</button>
                </div>
              </div>
            )
          })}
