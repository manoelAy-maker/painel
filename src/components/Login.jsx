...
const TEXTOS = {
  pt: {
    ...
    localizacaoIndisponivel: 'Este celular não retornou GPS agora. O acesso será liberado e registrado com alerta.',
    localizacaoTitulo: 'Localização bloqueada',
    localizacaoTexto: 'Se você negou sem querer, libere a permissão do site e tente entrar novamente.',
    iphone: 'iPhone/Safari',
    iphonePassos: 'Toque em AA ou no cadeado da barra de endereço → Ajustes do site → Localização → Permitir. Se não aparecer, vá em Ajustes do iPhone → Privacidade e Segurança → Serviços de Localização → Safari.',
    android: 'Android/Chrome',
    androidPassos: 'Toque no cadeado ao lado do endereço → Permissões → Localização → Permitir. Depois volte para o AYRES e tente novamente.',
    tentarLocalizacao: 'Tentar novamente',
  ...
  en: {
    ...
    localizacaoIndisponivel: 'This phone did not return GPS now. Access will continue and be registered with an alert.',
    localizacaoTitulo: 'Location blocked',
    localizacaoTexto: 'If you denied by mistake, enable site permission and try again.',
    iphone: 'iPhone/Safari',
    iphonePassos: 'Tap AA or the lock icon in the address bar → Website Settings → Location → Allow. If needed, open iPhone Settings → Privacy & Security → Location Services → Safari.',
    android: 'Android/Chrome',
    androidPassos: 'Tap the lock icon near the address → Permissions → Location → Allow. Then return to AYRES and try again.',
    tentarLocalizacao: 'Try again',
...
function tentarGeolocalizacao(opcoes) {
  return new Promise((resolve, reject) => {
    if (!navigator.geolocation) {
      const erro = new Error('Geolocalização indisponível')
      erro.code = 0
      reject(erro)
      return
    }
...
async function pedirLocalizacaoObrigatoria(textos) {
  try {
    await tentarGeolocalizacao({ enableHighAccuracy: true, timeout: 12000, maximumAge: 0 })
    return { liberado: true, motivo: 'precisa' }
  } catch (err) {
    if (err?.code === 1) {
      const e = new Error(textos.localizacaoObrigatoria)
      e.code = 1
      throw e
    }
  }
  ...
  try {
    await tentarGeolocalizacao({ enableHighAccuracy: false, timeout: 7000, maximumAge: 120000 })
    return { liberado: true, motivo: 'aproximada' }
  } catch (err) {
    if (err?.code === 1) {
      const e = new Error(textos.localizacaoObrigatoria)
      e.code = 1
    ...
    return { liberado: true, motivo: 'falha-tecnica' }
  }
}

function LocationHelp({ textos, onRetry, busy }) {
  return (
    <div className="location-help-card">
      <strong>{textos.localizacaoTitulo}</strong>
      <p>{textos.localizacaoTexto}</p>
      <div className="location-help-steps">
        <div><b>{textos.iphone}</b><span>{textos.iphonePassos}</span></div>
        <div><b>{textos.android}</b><span>{textos.androidPassos}</span></div>
      </div>
      <button type="button" disabled={busy} onClick={onRetry}>{busy ? textos.localizando : textos.tentarLocalizacao}</button>
    </div>
  )
}
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    const localOk = await validarLocalizacao()
    if (!localOk) return
    setCarregando(true)
