...
const Svg = ({ children, ...p }) => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}>{children}</svg>
const ICONES = {
...
const TEXTOS = {
  pt: {
    marca: 'Logística inteligente', titulo: 'Bem-vindo ao', descricao: 'Entre com suas credenciais para acessar a plataforma. A localização é obrigatória para validar o acesso operacional.', cardTitulo: 'Acessar plataforma', cardSub: 'Informe suas credenciais de acesso', usuario: 'Usuário', usuarioPlaceholder: 'Seu ID de acesso', senha: 'Senha', senhaPlaceholder: '••••••••', esqueci: 'Esqueci minha senha', erro: 'Usuário ou senha inválidos.', validando: 'Validando...', localizando: 'Validando localização...', entrar: 'ENTRAR NA PLATAFORMA',
    localizacaoObrigatoria: 'Para acessar o AYRES, permita a localização do site. Sem localização o acesso fica bloqueado.', localizacaoIndisponivel: 'Este navegador não permite localização. Acesso bloqueado.', localizacaoTitulo: 'Localização bloqueada', localizacaoTexto: 'Se você negou sem querer, libere a permissão do site e tente entrar novamente.', iphone: 'iPhone/Safari', iphonePassos: 'Toque em AA ou no cadeado da barra de endereço → Ajustes do site → Localização → Permitir. Se não aparecer, vá em Ajustes do iPhone → Privacidade e Segurança → Serviços de Localização → Safari.', android: 'Android/Chrome', androidPassos: 'Toque no cadeado ao lado do endereço → Permissões → Localização → Permitir. Depois volte para o AYRES e tente novamente.', tentarLocalizacao: 'Tentar novamente',
    recuperacao: 'Recuperação', esqueciTitulo: 'Esqueci minha senha', esqueciTexto: 'O admin receberá uma notificação para autorizar a troca.', nome: 'Nome', nomePlaceholder: 'Seu nome', motivo: 'Motivo', motivoPlaceholder: 'Ex: esqueci minha senha', enviarPedido: 'Solicitar troca de senha', enviando: 'Enviando...', informeUsuario: 'Informe seu usuário.', pedidoEnviado: 'Pedido enviado. Aguarde o admin autorizar a troca de senha.', pedidoLocal: 'Pedido salvo neste aparelho. Avise o admin para verificar depois.', beneficios: ['Controle Avançado', 'Tempo Real', 'Inteligência de Dados'], direitos: '© 2026 AYRES Logística. Todos os direitos reservados.', acesso: <>Localização obrigatória<br />Acesso monitorado</>,
  },
  en: {
    marca: 'Smart logistics', titulo: 'Welcome to', descricao: 'Sign in with your credentials to access the platform. Location is required to validate operational access.', cardTitulo: 'Access platform', cardSub: 'Enter your access credentials', usuario: 'User', usuarioPlaceholder: 'Your access ID', senha: 'Password', senhaPlaceholder: '••••••••', esqueci: 'Forgot password', erro: 'Invalid user or password.', validando: 'Validating...', localizando: 'Validating location...', entrar: 'ACCESS PLATFORM',
    localizacaoObrigatoria: 'To access AYRES, allow site location. Without location, access is blocked.', localizacaoIndisponivel: 'This browser does not allow location. Access blocked.', localizacaoTitulo: 'Location blocked', localizacaoTexto: 'If you denied by mistake, enable site permission and try again.', iphone: 'iPhone/Safari', iphonePassos: 'Tap AA or the lock icon in the address bar → Website Settings → Location → Allow. If needed, open iPhone Settings → Privacy & Security → Location Services → Safari.', android: 'Android/Chrome', androidPassos: 'Tap the lock icon near the address → Permissions → Location → Allow. Then return to AYRES and try again.', tentarLocalizacao: 'Try again',
    recuperacao: 'Recovery', esqueciTitulo: 'Forgot my password', esqueciTexto: 'The admin will receive a notification to authorize the password change.', nome: 'Name', nomePlaceholder: 'Your name', motivo: 'Reason', motivoPlaceholder: 'Ex: I forgot my password', enviarPedido: 'Request password change', enviando: 'Sending...', informeUsuario: 'Enter your user ID.', pedidoEnviado: 'Request sent. Wait for admin approval.', pedidoLocal: 'Request saved on this device. Ask the admin to check it later.', beneficios: ['Advanced Control', 'Real Time', 'Data Intelligence'], direitos: '© 2026 AYRES Logistics. All rights reserved.', acesso: <>Location required<br />Monitored access</>,
  },
}

function pedirLocalizacaoObrigatoria(textos) {
  return new Promise((resolve, reject) => {
    if (!navigator.geolocation) { reject(new Error(textos.localizacaoIndisponivel)); return }
    navigator.geolocation.getCurrentPosition(
      () => resolve(true),
      (err) => {
        const e = new Error(err?.code === 1 ? textos.localizacaoObrigatoria : (err?.message || textos.localizacaoObrigatoria))
        e.code = err?.code
        reject(e)
      },
...
export default function Login() {
  ...
  const validarLocalizacao = async () => {
    ...
    setLocalizando(true)
    try { await pedirLocalizacaoObrigatoria(textos); setLocalizando(false); return true }
    catch (err) { setErro(err?.message || textos.localizacaoObrigatoria); setShowLocationHelp(true); setLocalizando(false); return false }
  }
