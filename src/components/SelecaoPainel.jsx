...
const ICONES = {
  ...
  truck: <Svg><path d="M1 5h14v11H1z" /><path d="M15 9h4l4 4v3h-8V9z" /><circle cx="5.5" cy="18.5" r="2.5" /><circle cx="18.5" cy="18.5" r="2.5" /></Svg>,
...
const cores = {
  ...
  emerald: { texto: 'text-emerald-300', iconeWrap: 'bg-emerald-500/10 border-emerald-400/20 text-emerald-300', glow: 'hover:shadow-[0_35px_90px_-28px_rgba(16,185,129,0.68)] hover:border-emerald-300/35', linha: 'from-emerald-400/75 to-teal-200/20' },
}
  ...
  return (
    <div className="portal-screen min-h-screen flex flex-col bg-[#05070a] text-white font-sans relative overflow-hidden" style={{ fontFamily: "'Inter','Plus Jakarta Sans',Arial,sans-serif" }}>
      ...
      <header className="px-6 sm:px-10 py-7 flex justify-between items-center max-w-7xl mx-auto w-full">
        <div className="flex items-center gap-4"><div className="portal-logo-mark"><AyresLogo size={48} /></div><div><h1 className="text-2xl font-black tracking-tight">AYRES</h1><p className="text-[10px] uppercase tracking-[0.3em] text-blue-300 font-black">{t.marca}</p></div></div>
        <div className="flex items-center gap-3"><div className="hidden sm:flex items-center gap-2 rounded-full border border-white/10 bg-white/[.045] px-4 py-2 text-xs text-slate-300 backdrop-blur-xl">{ICONES.user}<span>{usuarioAtual?.nome || usuarioAtual?.usuario}</span></div><button type="button" onClick={sair} className="bg-white/[.045] border border-white/10 rounded-full px-4 py-2.5 text-xs font-bold flex items-center gap-2 hover:bg-red-500/10 hover:border-red-400/20 transition-all text-red-300">{ICONES.logout}<span className="hidden sm:inline">{t.logout}</span></button></div>
      </header>
      <main className="flex-1 flex items-center justify-center px-6 py-8">
        <section className={`w-full max-w-7xl mx-auto transition-all duration-1000 ease-out ${heroVisivel ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="text-center mb-9 sm:mb-12"><div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[.045] px-4 py-2 text-[11px] font-black uppercase tracking-[.16em] text-slate-300 mb-6 backdrop-blur-xl">{ICONES.shield} {filialLabel}</div><h2 className="text-4xl sm:text-6xl font-black tracking-tight leading-tight mb-4">{t.saudacao}, <span className="text-blue-300">{primeiroNome}</span></h2><p className="text-slate-400 text-base sm:text-lg">{t.escolher}</p></div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            {t.modulos.map((m) => { const c = cores[m.cor]; return <button key={m.id} type="button" onClick={() => acessar(m)} onMouseMove={moverGlow} className={`group relative overflow-hidden text-left p-8 sm:p-10 min-h-[390px] rounded-[2.5rem] border border-white/10 bg-[#0c111a]/90 backdrop-blur-xl cursor-pointer transition-all duration-500 hover:-translate-y-2 ${c.glow}`}><span className={`absolute left-8 right-8 top-0 h-px bg-gradient-to-r ${c.linha}`} /><span aria-hidden="true" className="pointer-events-none absolute inset-0 rounded-[2.5rem] opacity-0 transition-opacity duration-300 group-hover:opacity-100" style={{ background: `radial-gradient(circle at var(--x, 50%) var(--y, 50%), ${m.cor === 'blue' ? 'rgba(59,130,246,.16)' : m.cor === 'orange' ? 'rgba(249,115,22,.16)' : 'rgba(16,185,129,.16)'}, transparent 43%)` }} /><div className="relative flex min-h-[310px] flex-col justify-between"><div><div className="flex items-center justify-between gap-5 mb-10"><div className={`w-16 h-16 rounded-3xl flex items-center justify-center border ${c.iconeWrap}`}>{ICONES[m.icon]}</div><span className="rounded-full border border-white/10 bg-white/[.045] px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-400">{m.etiqueta}</span></div><h3 className="text-4xl sm:text-5xl font-black mb-5 tracking-tight">{t.painel} <span className={c.texto}>{m.nome}</span></h3><p className="text-slate-400 text-base leading-relaxed max-w-md">{m.subtitulo}</p></div><div><div className="flex flex-wrap gap-2 mb-6">{m.bullets.map((b) => <span key={b} className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1.5 text-xs font-bold text-slate-300">{b}</span>)}</div><div className={`flex items-center justify-between rounded-2xl border border-white/10 bg-white/[.045] px-5 py-4 font-black text-sm ${c.texto}`}><span>{t.entrar}</span>{ICONES.arrowRight}</div></div></div></button> })}
          </div>
