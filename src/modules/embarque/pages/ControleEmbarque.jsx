...
function lotePayload(lote, usuarioAtual) {
  ...
  return {
    local_id: String(lote.id), filial_id: filial, produto: lote.produto || null, origem: lote.origem || null, destino: lote.destino || null, lote: lote.lote || null,
    peso_lote: numeroPeso(lote.pesoLote) || null, cadencia: Math.max(0, Number(lote.cadencia || 0) || 0), data_inicio: hojeISO(), data_fim: null,
...
function linhaPayload(linha, lote, usuarioAtual) {
  const filial = lote?.filial || usuarioAtual?.filial || 'jatai-go'
  return {
    local_id: String(linha.id), lote_local_id: String(linha.loteId), data_operacao: linha.dia, cota: Number(linha.cota || 0) || null, filial_id: filial,
    placa: placaLimpa(linha.placa), peso: numeroPeso(linha.peso) || null, ordem_carregamento: linha.ordem ? 'X' : '', cte: linha.cte ? 'X' : '',
    ...
    produto: lote?.produto || null, origem: lote?.origem || null, destino: lote?.destino || null, lote: lote?.lote || null, responsavel_usuario: null, observacao: linha.observacao || null,
...
export default function ControleEmbarque() {
  ...
  const resumoDia = useMemo(() => {
    ...
    return {
      ...
      caminhoes: linhasDia.length,
      carregados: linhasDia.filter(l => l.status === 'carregado').length,
  ...
  async function salvarLote() {
    ...
    setLoteAberto(item.id); limparFormLote()
    ...
    <section className="embarque-dia-page">
      <header className="embarque-dia-hero">
        <div>
          ...
          <h1>Hoje primeiro, igual planilha</h1>
          <p>Abre no dia atual e mostra todos os lotes disponíveis para carregar.</p>
        </div>
        <div className="hero-actions-dia">
          <div className="hoje-box"><small>{diaAtivo === hojeISO() ? 'HOJE' : 'DIA'}</small><strong>{dataBR(diaAtivo, { day: '2-digit', month: '2-digit' })}</strong></div>
      ...
      <nav className="dia-nav-top">
        <button onClick={() => setDiaAtivo(addDias(diaAtivo, -1))}>‹ Dia anterior</button>
        {diasTopo.map(d => <button key={d} className={d === diaAtivo ? 'active' : ''} onClick={() => setDiaAtivo(d)}>{d === hojeISO() ? 'Hoje' : dataBR(d, { day: '2-digit' })}</button>)}
        <button onClick={() => setDiaAtivo(addDias(diaAtivo, 1))}>Próximo dia ›</button>
      </nav>

      <section className="lote-form dia-card"><div className="form-title"><strong>{editandoLoteId ? 'Editar lote' : 'Novo lote'}</strong><small>Peso em toneladas</small></div><input placeholder="Produto" value={formLote.produto} onChange={e => setFormLote(f => ({ ...f, produto: e.target.value.toUpperCase() }))} /><input placeholder="Origem" value={formLote.origem} onChange={e => setFormLote(f => ({ ...f, origem: e.target.value.toUpperCase() }))} /><input placeholder="Destino" value={formLote.destino} onChange={e => setFormLote(f => ({ ...f, destino: e.target.value.toUpperCase() }))} /><input placeholder="Lote" value={formLote.lote} onChange={e => setFormLote(f => ({ ...f, lote: e.target.value }))} /><input type="number" step="0.001" placeholder="Peso lote ton" value={formLote.pesoLote} onChange={e => setFormLote(f => ({ ...f, pesoLote: e.target.value }))} /><input type="number" placeholder="Cadência" value={formLote.cadencia} onChange={e => setFormLote(f => ({ ...f, cadencia: e.target.value }))} /><div className="form-actions"><button className="primary-btn" onClick={salvarLote}>{editandoLoteId ? 'Salvar lote' : 'Criar lote'}</button>{editandoLoteId && <button className="ghost-btn light" onClick={limparFormLote}>Cancelar</button>}</div></section>

      <section className="dia-metrics">
        <article><strong>{resumoDia.lotes}</strong><span>Lotes disponíveis</span></article>
        <article><strong>{resumoDia.caminhoes}</strong><span>Caminhões hoje</span></article>
      ...
      <div className="dia-layout">
        <aside className="lotes-mini-list">
          <strong>{mostrarArquivados ? 'Arquivados' : 'Lotes do dia'}</strong>
          <div>{lotesDia.map(l => { const st = statsLote(l); return <button key={l.id} onClick={() => setLoteAberto(String(loteAberto) === String(l.id) ? '' : l.id)} className={String(loteAberto) === String(l.id) ? 'active' : ''}><span>{l.produto} · {l.lote}</span><small>{st.listaDia.length} cam. · falta {formatarPeso(st.restante)}t</small></button> })}</div>
        </aside>

        <main className="lotes-dia-area">
          {lotesDia.map(lote => {
            const st = statsLote(lote)
            const aberto = String(loteAberto || lotesDia[0]?.id) === String(lote.id)
            return <section className="lote-dia-bloco" key={lote.id}>
              <div className="lote-dia-head" onClick={() => setLoteAberto(aberto ? '' : lote.id)}>
                <div className="lote-title-dia"><strong>{lote.produto} · LOTE {lote.lote}</strong><span>{lote.origem} → {lote.destino} · Cadência {lote.cadencia}</span><div className="progress-line"><span style={{ width: `${st.progresso}%` }} /></div></div>
                <div><small>Peso lote</small><strong>{formatarPeso(st.pesoLote)} ton</strong></div>
                <div><small>Carregado</small><strong>{formatarPeso(st.pesoCarregadoLote)} ton</strong></div>
                <div><small>Restante</small><strong>{formatarPeso(st.restante)} ton</strong></div>
                <div className="lote-actions-dia"><button onClick={(ev) => { ev.stopPropagation(); editarLote(lote, ev) }}>Editar</button>{lote.status === 'ativo' ? <button onClick={(ev) => arquivarLote(lote, ev)}>Arquivar</button> : <button onClick={(ev) => reabrirLote(lote, ev)}>Reabrir</button>}<button className="danger" onClick={(ev) => excluirLote(lote, ev)}>Excluir</button></div>
              </div>

              {aberto && <div className="lote-dia-corpo">
                <div className="lote-dia-actions"><button onClick={() => gerarCotasDoDia(lote)}>Gerar cotas do dia</button><button onClick={() => adicionarLinha(lote)}>+ Adicionar placa</button><span>{st.carregadosDia} carregados · {st.aguardandoDia} aguardando · {formatarPeso(st.pesoDia)} ton hoje</span></div>
                <div className="excel-wrap"><table className="excel-table"><thead><tr><th>COTA</th><th>PLACA</th><th>PESO</th><th>ORDEM</th><th>CTE</th><th>TRANSP</th><th>RESPONSÁVEL</th><th>STATUS</th><th>SEGURADORA</th><th>CHAMADO MDFE</th><th>AÇÕES</th></tr></thead><tbody>{st.listaDia.sort((a, b) => (Number(a.cota || 0) - Number(b.cota || 0))).map(l => <tr key={l.id} className={`linha-${STATUS[l.status]?.classe || 'agendado'}`}><td><input value={l.cota} onChange={e => atualizarLinha(l.id, 'cota', e.target.value)} onBlur={() => persistirLinha(l.id)} /></td><td><input value={l.placa} onChange={e => atualizarLinha(l.id, 'placa', placaLimpa(e.target.value))} onBlur={() => persistirLinha(l.id)} /></td><td><input value={l.peso} onChange={e => atualizarLinha(l.id, 'peso', e.target.value)} onBlur={() => persistirLinha(l.id)} placeholder="0,000" /></td><td><button className={l.ordem ? 'x-on' : ''} onClick={() => salvarLinha({ ...l, ordem: !l.ordem }, lote)}>X</button></td><td><button className={l.cte ? 'x-on' : ''} onClick={() => salvarLinha({ ...l, cte: !l.cte }, lote)}>X</button></td><td><input value={l.transportadora} onChange={e => atualizarLinha(l.id, 'transportadora', e.target.value.toUpperCase())} onBlur={() => persistirLinha(l.id)} /></td><td><input value={l.responsavel} onChange={e => atualizarLinha(l.id, 'responsavel', e.target.value.toUpperCase())} onBlur={() => persistirLinha(l.id)} /></td><td><select value={l.status} onChange={e => salvarLinha({ ...l, status: e.target.value }, lote)}><option value="agendado">AG. CARREGAMENTO</option><option value="carregado">CARREGADO</option><option value="cancelado">CANCELADO</option></select></td><td><input value={l.seguradora} onChange={e => atualizarLinha(l.id, 'seguradora', e.target.value.toUpperCase())} onBlur={() => persistirLinha(l.id)} /></td><td><input value={l.chamadoMdfe} onChange={e => atualizarLinha(l.id, 'chamadoMdfe', e.target.value.toUpperCase())} onBlur={() => persistirLinha(l.id)} /></td><td><button onClick={() => marcarCarregado(l)}>Carregou</button><button onClick={() => apagarLinha(l.id)}>Excluir</button></td></tr>)}{!st.listaDia.length && <tr><td colSpan="11" className="vazio">Nenhuma placa hoje. Use “Adicionar placa” ou “Gerar cotas do dia”.</td></tr>}</tbody><tfoot><tr><td colSpan="2">TOTAL DO DIA</td><td>{formatarPeso(st.pesoDia)}</td><td colSpan="8">{st.carregadosDia} carregados · {st.aguardandoDia} aguardando · restante lote {formatarPeso(st.restante)} ton</td></tr></tfoot></table></div>
              </div>}
            </section>
          })}
          {!lotesDia.length && <div className="empty-main">Nenhum lote disponível neste filtro.</div>}
        </main>
      </div>
    </section>
  )
